...
<!--        <xsl:variable name="svg" select="if(doc-available($svg-file)) then doc($svg-file) else () " />-->
    ...
    <xsl:variable name="svg-rank-distance" >
        ...
        <xsl:variable name="xs">
            <xsl:for-each-group select="$svg-elements/svg:text/@x" group-by=".">
                ...
                <xsl:sequence select="number(.)"></xsl:sequence>
            </xsl:for-each-group>
        </xsl:variable> 
        ...
        <xsl:variable name="diffs">
            <xsl:for-each select="$xs-seq[position() !=last()]" >
                ...
                <xsl:sequence select="number($xs-seq[$pos + 1]) - number(current())"></xsl:sequence>
            </xsl:for-each>
        </xsl:variable>
    ...
    <xsl:template match="/" >
        <xsl:apply-templates select="graph" mode="json-d3" ></xsl:apply-templates>
    </xsl:template>
...
<!--           <xsl:if test="not(position()=last())"><xsl:text>, </xsl:text></xsl:if>            -->
    ...
    <!-- adding initial x based on level -->
     ...
     <xsl:text>"init_x":</xsl:text><xsl:value-of select="if ($svg-element) then floor($svg-element/svg:text/@x div $svg-rank-distance  * $rank-distance)   else @level * $rank-distance" />
<!--    <xsl:text>, "px":</xsl:text><xsl:value-of select="@level * $rank-distance" />-->
     <xsl:text>, "init_y":</xsl:text><xsl:value-of select="if ($svg-element) then round($svg-element/svg:text/@y div -50) else @level * $rank-distance" />
...
<xsl:template match="edges/*">    
    <xsl:text>{"source":</xsl:text><xsl:value-of select="@ix_from" /><xsl:text>, "target":</xsl:text><xsl:value-of 
                        select="@ix_to" /><xsl:text>, "value":</xsl:text><xsl:value-of select="@value" />
